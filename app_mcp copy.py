...
AZURE_OPENAI_PROMPT = os.getenv("AZURE_OPENAI_PROMPT_MCP")
...
async def chat_endpoint(request: Request):
    ...
    try:
        ...
        # Use async with for MCP tools as context managers
        async with (
            MCPStdioTool(
                name="ai-search", 
                command="python", 
            ...
            tools = [ai_search, ms_learn, tavily]
            result = await agent.run(message, thread=thread, tools=tools)
        ...
        async def event_generator():
            try:
                ...
                async with (
                    ...
                    # Stream the response
                    async for chunk in agent.run_stream(message, thread=thread, tools=tools):
                        if chunk.text:
...
if __name__ == "__main__":
    ...
    print("\nMCP Tools:")
    print("  - AI Search MCP (Stdio)")
    print("  - Microsoft Learn MCP (HTTP)")
